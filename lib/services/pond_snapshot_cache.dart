...
class PondSnapshotCache {
  PondSnapshotCache({MockSensorService? sensorService}) : _sensorService = sensorService ?? MockSensorService();

  final MockSensorService _sensorService;
  final Map<String, PondSnapshot> _snapshots = {};

  PondSnapshot snapshotFor(Pond pond) {
    return _snapshots.putIfAbsent(pond.id, () => _sensorService.generateSnapshot(pond));
  }
